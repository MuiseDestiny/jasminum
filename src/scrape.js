export default {
    ...
    setCookieSandbox: function () {
        ...
        Zotero.Jasminum.cookieSandbox = new Zotero.CookieSandbox("", url, cookieData, userAgent);
    },
    ...
    setRefCookieSandbox: function () {
        ...
        var userAgent = Zotero.Jasminum.userAgent;
        var url = "https://cnki.net/";
        Zotero.Jasminum.refCookieSandbox = new Zotero.CookieSandbox("", url, cookieData, userAgent);
    },
     ...
     */
    setAttachmentCookieSandBox: function () {
        var cookieData = Zotero.Prefs.get("jasminum.cnki.attachment.cookie");
        var userAgent = Zotero.Jasminum.userAgent;
        var url = "https://cnki.net/";
        Zotero.Jasminum.attachmentCookieSandbox = new Zotero.CookieSandbox("", url, cookieData, userAgent);
    },
     ...
     * @return {String} 
     */
    createRefPostData: function (ids) {
     ...
     */
    getCNKIID: async function (url) {
        if (Zotero.Jasminum.Scrape.getIDFromURL(url)) {
            return Zotero.Jasminum.Scrape.getIDFromURL(url);
        } else {
            let htmlString = await Zotero.Jasminum.Scrape.getHtmlPage(url);
            let htmlDocument = Zotero.Jasminum.Utils.string2HTML(htmlString);
            return Zotero.Jasminum.Scrape.getIDFromPage(htmlDocument);
        }
    },
    ...
    search: async function (fileData) {
        ...
        var postData = Zotero.Jasminum.Scrape.createPostData(fileData);
        var requestHeaders = {
        ...
        if (!Zotero.Jasminum.Scrape.CookieSandbox) {
            Zotero.Jasminum.Scrape.setCookieSandbox();
        }
        // Zotero.debug(Zotero.Jasminum.cookieSandbox);
        var resp = await Zotero.HTTP.request("POST", postUrl, {
            headers: requestHeaders,
            cookieSandbox: Zotero.Jasminum.Scrape.CookieSandbox,
            body: postData,
        });
        // Zotero.debug(resp.responseText);
        var targetRows = Zotero.Jasminum.Scrape.getItemFromSearch(resp.responseText);
        return targetRows;
    },


    getItemFromSearch: function (resptext) {
        Zotero.debug("**Jasminum get item from search");
        var html = Zotero.Jasminum.Utils.string2HTML(resptext);
        ...
        if (!rows.length) {
            ...
            var targetIndicator = Zotero.Jasminum.Scrape.selectRow(rowIndicators);
    ...
    getCitationFromSearch: function (targetRow) {
     ...
     */
    getCitationFromPage: function (html) {
     ...
     * @return {String}
     */
    getRefText: async function (postData) {
        let url = "https://kns.cnki.net/KNS8/manage/ShowExport";
        if (!Zotero.Jasminum.Scrape.RefCookieSandbox) {  // Zotero.Jasminum is may be error
    ...
    getRefworks: async function (targetRows) {
        ...
        postData = Zotero.Jasminum.Scrape.createRefPostData(targetIDs);
        Zotero.debug(postData);
        var data = await Zotero.Jasminum.Scrape.getRefText(postData);
    ...
    getReaderUrl: function (itemUrl) {
        ...
        var itemid = Zotero.Jasminum.Scrape.getIDFromURL(itemUrl);
        var readerUrl =
    ...
    getChapterUrl: async function (readerUrl) {
        ...
        var readerHTML = Zotero.Jasminum.Utils.string2HTML(reader.responseText);
        return (
    ...
    getBookmark: async function (item) {
        ...
        if (  // CNKI reader URL exists
            ...
            Zotero.Jasminum.Scrape.getIDFromURL(parentItem.getField("url")) // A valid ID
        ) {
            Zotero.debug("** Jasminum item url exists");
            itemUrl = parentItem.getField("url");
            itemReaderUrl = Zotero.Jasminum.Scrape.getReaderUrl(itemUrl);
        } else {
        ...
        itemChapterUrl = await Zotero.Jasminum.Scrape.getChapterUrl(itemReaderUrl);
        Zotero.debug("** Jasminum item chapter url: " + itemChapterUrl);
        // Next line raises: Invalid chrome URI: /
        var out = Zotero.Jasminum.Scrape.getChapterText(itemChapterUrl, item);
        return out;
    },
    ...
    addBookmark: async function (item, bookmark) {
        ...
        if (Zotero.isWin) {
            ...
            if (!cacheFile.exists()) {
                cacheFile.create(
                    Components.interfaces.nsIFile.DIRECTORY_TYPE, 0o777  // 八进制转换
                );
            }
        }
     ...
     * @return {void}
     */
    importAttachment: async function (item) {
        Zotero.debug("** Jasminum import attachment begin");
        let articleID = await Zotero.Jasminum.Scrape.getCNKIID(item.getField("url"));
        ...
        let articleUrl = Zotero.Jasminum.Utils.getURLFromID(articleID, en = true);
        let attachmentUrl = await Zotero.Jasminum.Scrape.getAttachmentURL(articleUrl)
        ...
        if (!Zotero.Jasminum.Scrape.attachmentCookieSandbox) {
            Zotero.Jasminum.Scrape.setAttachmentCookieSandBox();
        }
        var importOptions = {
            ...
            cookieSandbox: Zotero.Jasminum.Scrape.attachmentCookieSandbox,
        };
    ...
    */
    getAttachmentURL: async function (articleUrl) {
     ...
     */
    getVlistFromPage: function (html) {
        return html.querySelector("input#listv").value;
    },


    getRefIDs: async function (url) {
        let htmlString = await Zotero.Jasminum.Scrape.getHtmlPage(url);
        let htmlDocument = Zotero.Jasminum.Utils.string2HTML(htmlString);
        let listv = Zotero.Jasminum.Scrape.getVlistFromPage(htmlDocument);
        let cnkiID = Zotero.Jasminum.Scrape.getIDFromPage(htmlDocument);
